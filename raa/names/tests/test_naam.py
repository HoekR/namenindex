...
#            (Name('koning Willem III') , 'Willem III' ), 
            (Name('Crato, graaf van Nassau-Saarbrück'), 'Crato, graaf van Nassau-Saarbrück'),
            (Name('Wilhelmina van Pruisen - prinses van Oranje-Nassau'), 'Wilhelmina van Pruisen - prinses van Oranje-Nassau'),
            (Name('Henriette Adriana Louise Flora d\'Oultremont de Wégimont'), "d'Oultremont de Wégimont, Henriette Adriana Louise Flora"),
            (Name('Wolrat, vorst van Nassau-Usingen dikwijls genoemd Nassau-Saarbrück'), 'Wolrat, vorst van Nassau-Usingen dikwijls genoemd Nassau-Saarbrück'),
            (Name('van \'s-Gravezande, Arnoldus Corneluszn. Storm'), 's-Gravezande, Arnoldus Corneluszn. Storm, van'),
    ...
    def test_html_codes(self):
        n = Name('W&eacute;l?')
        n.html2unicode()
        self.assertEqual( n.volledige_naam(), 'Wél?')
...
#            self.assertEqual(Name(s)._strip_tussenvoegels(s), result)
    ...
    def test_to_string(self):
        self.assertEqual(Name('abc').store_guessed_geslachtsnaam().to_string(),
            '<persName><name type="geslachtsnaam">abc</name></persName>')
...
#        self.assertEqual(type(n.guess_normal_form()), type(u'Willem II'))
    ...
    def test_diacritics(self):
        n = Name('Wét').store_guessed_geslachtsnaam()
        el = etree.Element('test') #@UndefinedVariable
        el.text = 'Wét'
        s = '<persName><name type="geslachtsnaam">W\xe9t</name></persName>'
        self.assertEqual(n.to_string(), s)
...
<name type="voornaam">Hendrik IV</name>
    ...
    def test_fix_capitals(self):
        ...
        self.assertEqual(fix_capitals('Schepper, Gerhard Antoni IJssel de'), 'Schepper, Gerhard Antoni IJssel de')
                         
        
    def test_html2unicode(self): 
        s = 'M&ouml;törhead'
        n = Name(s)
        self.assertEqual(n.volledige_naam(), 'Mötörhead')
        
        #this shoudl not be here, but under a separate test for the utility functions in common
        self.assertEqual(html2unicode('&eacute;'), 'é')
        self.assertEqual(html2unicode('S&atilde;o'), 'São')
...
#      
    def test_initials(self):
        ...
        self.assertEqual(Name('Hoeven, Abraham des Amorie van der (1)').initials(), 'AAH')
...
#        self.assertEqual(Name('Jhr. Mr. K').soundex_nl(), ['k'])
        self.assertEqual(set(Name('janssen, hendrik').soundex_geslachtsnaam()), set(['j.ns']))
...
#        self.assertEqual(name_parts, [u'abc.', u'DE.', 'F;dk.', 'Genoeg-Van'])
    ...
    def test_constituent_tokens(self):    
        ...
        t1 = [('Menc\xeda', 'voornaam'), ('de', 'intrapositie'), ('Mendoza', 'geslachtsnaam'), ('y', 'geslachtsnaam'), ('Fonseca', 'geslachtsnaam')]
        self.assertEqual(Name(s1)._guess_constituent_tokens(), t1)
        ...
        t1 = [('Henriette', 'voornaam'), ('Adriana', 'voornaam'), ('Louise', 'voornaam'), ('Flora', 'voornaam'), ("d'", 'geslachtsnaam'), ('Oultremont', 'geslachtsnaam'), ('de', 'intrapositie'), ('W\xe9gimont', 'geslachtsnaam')]
        self.assertEqual(Name(s1)._guess_constituent_tokens(), t1)
    ...
    def test_tokenize(self):
        ...
        self.assertEqual(Name("l'Abc")._tokenize(), [("l'", None), ('Abc', None)])
